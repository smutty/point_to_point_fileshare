...
import org.ccnx.ccn.apps.ccnchat.ChatGetFile;
import org.ccnx.ccn.apps.ccnchat.ChatPutFile;
/**
...
public final class CCNChatNet {
	...
	public synchronized void sendMessage(String message) throws ContentEncodingException, IOException {
		...
		if(message.startsWith("putfile"))
		{
			...
			temp[0]="ccnx:/temp.txt";
			temp[1]=m[1];
		boolean rawMode = true;
		int timeout = 60000;
		try{
			ChatPutFile.main(temp, rawMode, timeout);
		}catch(IOException e){
			rawMode = false;
			timeout = 10000;
			try{
				...
				System.out.println("Configuration exception in ChatGetFile: " + ee.getMessage());
				ee.printStackTrace();
				_callback.recvMessage("File share failure, cannot connect to the local repository.");
			}
		}
			//ChatPutFile.main(temp);
			_writeString.save("UPLOAD:I have UPDATED A FILE:");
			
			//JOptionPane.showConfirmDialog(null, "choose ", "choose one", JOptionPane.YES_NO_OPTION);  
		}
		else if(message.startsWith("getfile"))
		{
			...
			temp[0]="ccnx:/temp.txt";
			temp[1]=m[1];
			ChatGetFile.main(temp);
			
			//send a feedback to the others after got the file
			_writeString.save("I have got the file, Thank you for sharing!!!");
			
			//_callback.recvMessage("[Me" + DATE_FORMAT.format(now()) + "]:Get file successfully!\n");
		}
		else _writeString.save(message);
	}
	...
	public void sentFile(String filename, String name) throws ContentEncodingException, IOException {
		...
		_writeString.save("UPLOAD:I have sent "+name);
	...
	private void showMessage(String sender, Timestamp time, String message) {
		...
		if(message.startsWith("UPLOAD:I have sent "))
		{
			
			
		int res;
		res = JOptionPane.showConfirmDialog(null, sender + " HAS UPLOAD A FILE, DO YOU WANT TO SAVE THIS FILE?", "ALERT", JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);  
		   ...
		   if(res==JOptionPane.YES_OPTION)
		 {
			   File file = null;
				...
				if(result==JFileChooser.APPROVE_OPTION)
				{
					...
					String temp[] = new String[2];
					temp[0]="ccnx:/temp.txt";
					temp[1]=file.getAbsolutePath();
					ChatGetFile.main(temp);
					
					//send a feedback to the others after you got a file
