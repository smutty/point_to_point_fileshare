...
import org.ccnx.ccn.utils.CommonOutput;
import org.ccnx.ccn.utils.CommonParameters;
import org.ccnx.ccn.utils.CommonArguments;

/**
 * Command-line utility to write a file to ccnd; requires a corresponding ChatPutFile
 * to pull the data or it will not move (flow balance).
 **/
 public class ChatPutFile extends CommonOutput implements Usage {
     static ChatPutFile ChatPutFile = new ChatPutFile();
 
	/**
	 * @param args
	 */
	public void write(String[] args, boolean rawMode, int timeout) throws IOException {
		...
		for (int i = 0; i < args.length - 2; i++) {
			if (args[i].equals("-local")) {
				...
				if (!CommonArguments.parseArguments(args, i, ChatPutFile)) {
		...
		//set the mode according to the given arguments
		CommonParameters.rawMode = rawMode;
		CommonParameters.timeout = timeout;
		...
		try {
			...
			if (args.length == (CommonParameters.startArg + 2)) {
				if (CommonParameters.verbose)
					Log.info("ChatPutFile: putting file " + args[CommonParameters.startArg + 1]);
				
				doPut(handle, args[CommonParameters.startArg + 1], argName);
				System.out.println("Inserted file " + args[CommonParameters.startArg + 1] + ".");
				if (CommonParameters.verbose)
					System.out.println("ChatPutFile took: "+(System.currentTimeMillis() - starttime)+" ms");
				//System.exit(0);
			} else {
	...
	public void usage(String extraUsage) {
		System.out.println("usage: ChatPutFile [-v (verbose)] [-raw] [-unversioned] [-local | -allownonlocal] [-timeout millis] [-log level] [-as pathToKeystore] [-ac (access control)] <ccnname> (<filename>|<url>)*");
		//System.exit(1);
	}

	public static void main(String[] args, boolean rawMode, int timeout) throws IOException{
		ChatPutFile.write(args, rawMode, timeout);
	}
}
