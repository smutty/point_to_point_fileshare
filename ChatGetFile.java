 ...
 */
public class ChatGetFile implements Usage {
    static Usage u = new ChatGetFile();
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		...
		try {
			...
			if (CommonParameters.verbose)
				System.out.println("ChatGetFile took: "+(System.currentTimeMillis() - starttime)+"ms");
			System.out.println("Retrieved content " + args[CommonParameters.startArg + 1] + " got " + readtotal + " bytes.");
			//System.exit(0);

		} catch (ConfigurationException e) {
			System.out.println("Configuration exception in ChatGetFile: " + e.getMessage());
			e.printStackTrace();
	...
	public void usage(String extraUsage) {
		System.out.println("usage: ChatGetFile " + extraUsage + "[-unversioned] [-timeout millis] [-as pathToKeystore] [-ac (access control)] <ccnname> <filename>");
	}
